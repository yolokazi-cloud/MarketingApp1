...
const UploadHistoryModal = ({ isOpen, onClose, dataType, onDataUpdate }) => {
  ...
  const fetchVersions = async () => {
    ...
    setError('');
    try {
      const response = await fetch(`http://localhost:3001/api/uploads/${dataType}/versions`);
  ...
  const handleViewVersion = async (versionId) => {
    ...
    setError('');
    try {
      const response = await fetch(`http://localhost:3001/api/uploads/${dataType}/versions/${versionId}`);
  ...
  const handleDownloadVersion = async (version) => {
    try {
      ...
      if (!versionData) {
        const response = await fetch(`http://localhost:3001/api/uploads/${dataType}/versions/${version._id}`);
  ...
  const handleSaveRecord = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:3001/api/uploads/${dataType}/versions/${selectedVersion._id}/records/${editingRecord.index}`, {
        method: 'PUT',
